...
## 1. activity at time_to_set_std = 0
df1<- data_new %>% 
  filter(time_to_set_std == 0) %>% 
...
data_new_lst<- lapply(data_new_lst, function(x){
  cbind(x, auc = DescTools::AUC(x=x$time_to_set_std, y=x$mu))
})
...
## a visual inspection of the produced values for the steepest ascend and descend showed that the range of daytime (in hours to sunrise) in which the steepest ascend or descend is chosen as activity start or end, differs for some days (the activity starts later or earlier, compared to average), these days must be filtered separately to get the correct values --> The highest "diff" is not always within the range of  [time_to_set_std >= -1.5 & time_to_set_std <= 2.25] or [time_to_set_std >= 13 & time_to_set_std <= 17]

# steepest_ascend:
df5<- data_new_diff %>% 
  group_by(ring_ID, date_f) %>% 
  filter(time_to_set_std >= -1.5 & time_to_set_std <= 2.25) %>% 
  filter(diff == max(diff,na.rm=TRUE)) %>% 
  summarise(steepest_ascend = time_to_set_std) %>% 
  as.data.frame() %>% 
...
df6<- data_new_diff %>% 
  group_by(ring_ID, date_f)  %>% 
  filter(time_to_set_std >= 13 & time_to_set_std <= 17)%>% 
  filter(diff == min(diff,na.rm=TRUE)) %>% 
  summarise(steepest_descend = time_to_set_std) %>% 
  as.data.frame() %>%  
...
sunset<- data_new[which(abs(data_new$time_to_set_std - 15.51328) == min(abs(data_new$time_to_set_std - 15.51328)))]$time_to_set_std[1]
df7<- data_new %>% 
  filter(time_to_set_std == sunset) %>% 
  group_by(ring_ID, date_f) %>% 
