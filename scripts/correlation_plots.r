...
df_10min <- df_1min %>% 
  mutate(species_en = as.factor(species_en),
         interval   = as_hms(floor_date(time_to_rise, unit="10minutes"))) %>% 
  group_by(ID, ring_ID, rdate, species_en,year_f,brood_patch,sex, month,week,ydate, date_f,hour,age, sex, weight, interval) %>% 
...
df_10min.r <- df_10min %>%  
  group_by(ring_ID, interval) %>% # year, year_f, 
...
x<- cor(df_10min.r[,2:67], use = "pairwise.complete.obs")
na.omit()corrplot::corrplot(x,method = "ellipse", type = "lower")


x<- corrr::correlate(df_10min.r[,2:67])#corrr::shave() %>% 
